...
### Architecture Fit
- **Components affected:** `pint.json` (new), `.claude/settings.json` (new), `.claude/hooks/run-pint.sh` (new), `.github/workflows/tests.yml` (modify), `composer.json` (modify), `CLAUDE.md` (modify), `docs/reference/spec/decisions-index.md` (modify)
...
```
**Verification:** `grep -c 'D17[789]' docs/reference/spec/decisions-index.md` returns 3.
