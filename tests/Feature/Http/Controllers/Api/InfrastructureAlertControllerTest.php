...
it('returns 401 for unauthenticated users on infrastructure alerts index', function () {
    $this->getJson('/api/v1/dashboard/infrastructure-alerts')
        ->assertUnauthorized();
});

it('returns 403 for non-admin users on infrastructure alerts index', function () {
...
it('returns active infrastructure alerts for admin users', function () {
    $project = Project::factory()->enabled()->create();
...
it('returns empty data array when no active infrastructure alerts exist', function () {
    $project = Project::factory()->enabled()->create();
    $user = createInfraAlertAdmin($project);
...
it('returns 401 for unauthenticated users on infrastructure alert acknowledge', function () {
...
it('returns 403 for non-admin users on infrastructure alert acknowledge', function () {
    $project = Project::factory()->enabled()->create();
    $user = createInfraAlertRegularUser($project);
...
it('acknowledges an infrastructure alert for admin users', function () {
    $project = Project::factory()->enabled()->create();
    $user = createInfraAlertAdmin($project);
...
it('returns 404 for non-existent alert on infrastructure alert acknowledge', function () {
    $project = Project::factory()->enabled()->create();
    $user = createInfraAlertAdmin($project);
