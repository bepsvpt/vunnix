...
test('security:check-pat-rotation creates alert when PAT is old', function () {
...
test('security:check-pat-rotation reports no alert when PAT is fresh', function () {
    GlobalSetting::updateOrCreate(
        ['key' => 'bot_pat_created_at'],
...
test('security:check-pat-rotation reports no alert when no PAT date configured', function () {
    $this->artisan('security:check-pat-rotation')
        ->expectsOutputToContain('No PAT rotation alert needed')
        ->assertSuccessful();
