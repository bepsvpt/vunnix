...
test('cost-alerts:evaluate warns when team chat notification fails', function (): void {
    $project = Project::factory()->enabled()->create();
    ...
    // Current month: $150 (> 2 × $50 = $100) → triggers monthly_anomaly
    $task3 = Task::factory()->create(['project_id' => $project->id]);
    DB::table('task_metrics')->insert([
        'task_id' => $task3->id,
        'project_id' => $project->id,
    ...
    // Mock AlertEventService to throw on notifyCostAlert — exercises the catch block
    $this->mock(AlertEventService::class)
        ->shouldReceive('notifyCostAlert')
        ->andThrow(new \RuntimeException('Chat webhook unreachable'));

    $this->artisan('cost-alerts:evaluate')
        ->expectsOutputToContain('cost alert(s) created')
        ->expectsOutputToContain('Failed to send team chat')
        ->assertSuccessful();
