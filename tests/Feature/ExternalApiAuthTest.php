...
it('accesses GET /api/v1/ext/metrics/summary via API key', function () {
    $this->getJson('/api/v1/ext/metrics/summary', [
        'Authorization' => 'Bearer '.$this->apiKeyResult['plaintext'],
    ])->assertOk();
});

it('accesses GET /api/v1/ext/activity via API key', function () {
...
it('accesses GET /api/v1/ext/projects via session auth', function () {
    $this->actingAs($this->user)
...
it('returns 401 without any auth on ext/tasks', function () {
    $this->getJson('/api/v1/ext/tasks')
        ->assertStatus(401);
});

it('returns 401 without any auth on ext/projects', function () {
    $this->getJson('/api/v1/ext/projects')
        ->assertStatus(401);
});
