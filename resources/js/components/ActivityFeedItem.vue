...
const linkText = computed(() => {
    if (props.item.mr_iid) return `MR !${props.item.mr_iid}`;
    if (props.item.issue_iid) return `Issue #${props.item.issue_iid}`;
    if (props.item.conversation_id) return 'View conversation';
    return null;
});
</script>
