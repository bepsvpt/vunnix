...
function buildFilters() {
    const filters = {};
    if (filterReason.value) filters.reason = filterReason.value;
...
async function handleRetry(entryId) {
    if (!confirm('Retry this failed task? A new task will be created and queued.')) return;
...
async function handleDismiss(entryId) {
    if (!confirm('Dismiss this entry? It will be marked as acknowledged and hidden from the active list.')) return;
      ...
      <div v-else-if="admin.deadLetterDetail" class="space-y-4">
        <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
          <div class="flex items-center justify-between mb-3">
            <h3 class="text-sm font-medium">Entry #{{ admin.deadLetterDetail.id }}</h3>
            <span
          ...
          <dl class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
            <dt class="text-zinc-500 dark:text-zinc-400">Dead lettered at</dt>
            <dd>{{ formatDate(admin.deadLetterDetail.dead_lettered_at) }}</dd>
            <dt class="text-zinc-500 dark:text-zinc-400">Task type</dt>
            <dd>{{ admin.deadLetterDetail.task_record?.type || '—' }}</dd>
            <dt class="text-zinc-500 dark:text-zinc-400">Project ID</dt>
            <dd>{{ admin.deadLetterDetail.task_record?.project_id || '—' }}</dd>
            <dt class="text-zinc-500 dark:text-zinc-400">Attempts</dt>
        ...
        <!-- Error details -->
        <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
          <h4 class="text-sm font-medium mb-2">Error Details</h4>
        ...
        <div v-if="admin.deadLetterDetail.attempt_history?.length" class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
          <h4 class="text-sm font-medium mb-2">Attempt History</h4>
          <div class="space-y-2">
            <div
            ...
            >
              ...
              <p v-if="attempt.error" class="mt-0.5 text-zinc-600 dark:text-zinc-400">{{ attempt.error }}</p>
            </div>
          </div>
        </div>
