            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <select v-model="assignForm.role_id" class="w-full rounded-lg border border-zinc-300 px-3 py-1.5 text-sm dark:border-zinc-600 dark:bg-zinc-800" data-testid="assign-role">
                        <option v-for="r in rolesForProject(assignForm.project_id)" :key="r.id" :value="r.id">
                            {{ r.name }}
                        </option>
                    </select>
                    <p v-if="rolesForProject(assignForm.project_id).length === 0" class="mt-1 text-xs text-zinc-400">
                        No roles defined for this project. Create one in the Roles tab first.
                    </p>
                </div>
