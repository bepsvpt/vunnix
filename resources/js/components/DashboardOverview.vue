...
const successRateDisplay = computed(() => {
    if (!overview.value || overview.value.success_rate === null) return '—';
    return `${overview.value.success_rate}%`;
});

const recentActivityDisplay = computed(() => {
    if (!overview.value?.recent_activity) return 'No activity';
  ...
  <div data-testid="dashboard-overview">
    ...
    <div v-else-if="overview" class="space-y-6">
      ...
      <div class="grid grid-cols-3 gap-4">
        ...
        >
          <p class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Active Tasks</p>
        ...
        <div
          ...
          class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-4"
        >
          <p class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Recent Activity</p>
      ...
      <div>
        <h3 class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">Tasks by Type</h3>
        <div class="grid grid-cols-4 gap-4">
          ...
          >
            ...
            <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{{ card.label }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <div class="text-center text-zinc-400 dark:text-zinc-500">
        <p class="text-sm">No overview data available.</p>
      </div>
    </div>
  </div>
</template>
