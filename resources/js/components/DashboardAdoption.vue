...
const mrPercentDisplay = computed(() => {
    if (!adoption.value || adoption.value.ai_reviewed_mr_percent === null) return '—';
...
const tasksByTypeMonths = computed(() => {
    if (!adoption.value?.tasks_by_type_over_time) return [];
...
const aiMentions = computed(() => {
    if (!adoption.value?.ai_mentions_per_week) return [];
  ...
  <div data-testid="dashboard-adoption">
    ...
    <div v-else-if="adoption" class="space-y-6">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        >
          <p class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">AI-Reviewed MR %</p>
          ...
          <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400" data-testid="ai-reviewed-mr-count">
            <template v-if="adoption.ai_reviewed_mr_percent !== null">{{ mrCountDisplay }}</template>
            <template v-else>No merge requests yet</template>
          </p>
        </div>
        ...
        <div
          ...
          class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-4"
        >
          <p class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Chat Active Users</p>
      ...
      <div>
        <h3 class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">Tasks by Type Over Time</h3>
        <div
          ...
          <div class="overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700">
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
              ...
              <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700 bg-white dark:bg-zinc-900">
                ...
                >
                  <td class="px-4 py-2 text-sm text-zinc-900 dark:text-zinc-100">{{ row.month }}</td>
                  <td
                    v-for="typeKey in allTypeKeys"
                    :key="typeKey"
        ...
        >
          <p class="text-sm">No task data over time yet.</p>
        </div>
      </div>

      <!-- @ai mentions per week -->
      <div>
        <h3 class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">@ai Mentions per Week</h3>
        <div
        ...
        >
          <div class="overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700">
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
              <thead class="bg-zinc-50 dark:bg-zinc-800">
                <tr>
                  <th class="px-4 py-2 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase">Week</th>
                  <th class="px-4 py-2 text-right text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase">Mentions</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700 bg-white dark:bg-zinc-900">
                <tr
                ...
                >
                  <td class="px-4 py-2 text-sm text-zinc-900 dark:text-zinc-100">{{ entry.week }}</td>
                  <td class="px-4 py-2 text-sm text-right text-zinc-900 dark:text-zinc-100">{{ entry.count }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div
          ...
          class="text-center text-zinc-400 dark:text-zinc-500 py-4"
        >
          <p class="text-sm">No @ai mention data yet.</p>
        </div>
      </div>
    </div>
    ...
    >
      <div class="text-center text-zinc-400 dark:text-zinc-500">
        <p class="text-sm">No adoption data available.</p>
      </div>
    </div>
  </div>
</template>
