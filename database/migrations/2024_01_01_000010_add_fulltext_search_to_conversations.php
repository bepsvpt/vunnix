...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // Skip gracefully when dependencies are not available.
        if (DB::connection()->getDriverName() !== 'pgsql'
            || ! Schema::hasTable('agent_conversations')
        ...
        if (! Schema::hasColumn('agent_conversations', 'project_id')) {
        ...
        DB::statement('ALTER TABLE agent_conversations ADD COLUMN IF NOT EXISTS title_search tsvector');
        DB::statement('CREATE INDEX IF NOT EXISTS agent_conversations_title_search_idx ON agent_conversations USING GIN (title_search)');

        // Auto-update tsvector on INSERT/UPDATE
        DB::statement("
            CREATE OR REPLACE FUNCTION agent_conversations_title_search_update() RETURNS trigger AS $$
            BEGIN
        ...
        DB::statement('ALTER TABLE agent_conversation_messages ADD COLUMN IF NOT EXISTS content_search tsvector');
        DB::statement('CREATE INDEX IF NOT EXISTS agent_conversation_messages_content_search_idx ON agent_conversation_messages USING GIN (content_search)');

        // Auto-update tsvector on INSERT/UPDATE
        DB::statement("
            CREATE OR REPLACE FUNCTION agent_conversation_messages_content_search_update() RETURNS trigger AS $$
            BEGIN
    ...
    public function down(): void
    {
        ...
        if (Schema::hasTable('agent_conversations') && Schema::hasColumn('agent_conversations', 'project_id')) {
            Schema::table('agent_conversations', function (Blueprint $table): void {
